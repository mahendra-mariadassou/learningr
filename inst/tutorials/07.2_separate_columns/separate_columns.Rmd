...
You can ask `separate()` to convert the new columns to an appropriate data type by adding `convert = TRUE` to your `separate()` call. R will guess the most appropriate data type for each newly created column and convert it to that type. 
...
```{r fig1, echo = FALSE, out.width = "90%", fig.cap = "WHO codes"}
knitr::include_graphics(path = "images/codes.png")
```
...
who %>%
  select(-iso2, -iso3) %>% 
  pivot_longer(names_to = "codes", values_to = "n", 3:58) %>%
...
```{r out.width = "90%"}
knitr::include_graphics(path = "images/reshape.png")
```
